...
public class ParticleRing : MonoBehaviour {
    ...
    void Start() {   // 初始化粒子数组  
        ...
        var setparticle = particleSys.main;
        setparticle.startSpeed = 0;            // 粒子位置由程序控制  
        setparticle.startSize = size;          // 设置粒子大小  
        setparticle.loop = false;
        setparticle.maxParticles = count;      // 设置最大粒子量  
    ...
    void RandomlySpread() {
        for (int i = 0; i < count; ++i) {   // 随机每个粒子距离中心的半径，同时希望粒子集中在平均半径附近  
